  ...
  if (catalogContainer && sidebar) {
    ...
    categoryOrder.forEach((category) => {
      if (cataloguesByCategory[category]) {
        const categoryHTML = `
              <div class="sidebar-category" data-category="${category.replace(/\s+/g, "-")}">
                ${category}
              </div>
            `;
    ...
    categoryOrder.forEach((category) => {
      if (cataloguesByCategory[category]) {
        // Create category section with title and grid
        const categoryHTML = `
              <div class="category-section" id="${category.replace(/\s+/g, "-")}">
                <h2 class="category-title" id="${category.replace(/\s+/g, "-")}-title">${category}</h2>
                <div class="category-grid" id="${category.replace(/\s+/g, "-")}-grid">
        ...
        // Add items to this category's grid
        const categoryGrid = document.getElementById(`${category.replace(/\s+/g, "-")}-grid`);

        cataloguesByCategory[category].forEach((item) => {
          const bookItemHTML = `
                <div class="book-items" data-category="${category.replace(/\s+/g, "-")}">
                  <a href="${item.link}" target="_blank">
                    <div class="main-book-wrap">
                      <div class="book-cover">
                        <div class="book-inside"></div>
                        <div class="book-image">
                          <img src="${item.imageSrc}" alt="${item.altText || "Catalog Cover"}" />
              ...
              `;
          categoryGrid.insertAdjacentHTML("beforeend", bookItemHTML);
        });
      }
    });
  } else {
...
document.addEventListener("click", function (e) {
  if (e.target.classList.contains("sidebar-category")) {
    ...
    const items = document.querySelectorAll(`.book-items[data-category="${category}"]`);
    items.forEach((item) => {
